...
func (r *nodeReconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, err error) {
	...
	r.signer, err = signer.Create(types.NamespacedName{Namespace: r.watchNamespace,
	...
	if _, ok := node.GetAnnotations()[metadata.RebootAnnotation]; ok {
		r.log.Info("rebooting node", "name", node.Name)
		if err := r.safeReboot(ctx, node); err != nil {
			return ctrl.Result{}, fmt.Errorf("full instance reboot failed for node %s: %w", node.Name, err)
		}
	}
	return ctrl.Result{}, nil
}
...
func (r *nodeReconciler) safeReboot(ctx context.Context, node *core.Node) error {
	...
	nc, err := nodeconfig.NewNodeConfig(r.client, r.k8sclientset, r.clusterServiceCIDR, r.watchNamespace,
		instanceInfo, r.signer, nil, nil, r.platform)
	if err != nil {
	...
	if err := drain.RunCordonOrUncordon(drainer, node, true); err != nil {
		return fmt.Errorf("unable to cordon node %s: %w", node.Name, err)
	}
	if err := drain.RunNodeDrain(drainer, node.Name); err != nil {
		return fmt.Errorf("unable to drain node %s: %w", node.Name, err)
	}

	nc.Windows.Run("Restart-Computer -Force", true)

	// wait for WICD to remove Reboot annotation
	if err := metadata.WaitForRebootAnnotationRemoval(ctx, r.client, node.Name); err != nil {
		return err
	}

	if err := drain.RunCordonOrUncordon(drainer, node, false); err != nil {
		return fmt.Errorf("unable to uncordon node %s: %w", node.Name, err)
	}
	return nil
}
...
func (r *nodeReconciler) SetupWithManager(mgr ctrl.Manager) error {
	windowsNodePredicate := predicate.Funcs{
		CreateFunc: func(e event.CreateEvent) bool {
			return windowsNodeFilter(e.Object)
		},
		UpdateFunc: func(e event.UpdateEvent) bool {
			return windowsNodeFilter(e.ObjectNew)
		},
...
// windowsNodeFilter returns true if the given object is a Windows node
func windowsNodeFilter(obj runtime.Object) bool {
