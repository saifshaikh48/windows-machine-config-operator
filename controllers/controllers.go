...
func (r *instanceReconciler) instanceFromNode(node *core.Node) (*instances.InstanceInfo, error) {
	...
	username, err := decryptUsername(usernameAnnotation, privateKeyBytes)
	if err != nil {
...
// encryptUsername returns the encrypted value to be used as a username annotation
func encryptUsername(username string, key []byte) (string, error) {
	encryptedUsername, err := crypto.Encrypt(username, key)
	if err != nil {
		return "", err
	}
	// Make encrypted data compatible as JSON Patch request body.
	// Newline characters are marked with text placeholder in order to be easily reversed during decryption
	return strings.Replace(encryptedUsername, "\n", "<newline>", -1), nil
}

// decryptUsername returns the plaintext value of an encrypted username annotation
func decryptUsername(encrypted string, key []byte) (string, error) {
	// Convert data from JSON compatible representation to string value
	encrypted = strings.Replace(encrypted, "<newline>", "\n", -1)
