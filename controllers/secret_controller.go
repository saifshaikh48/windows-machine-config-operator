	...
	if err != nil && k8sapierrors.IsNotFound(err) {
		...
		for _, node := range nodes.Items {
			if _, present := node.Annotations[BYOHAnnotation]; present {
				...
				username, err := r.usernameFromNode(ctx, request, &node)
				if err != nil {
					return reconcile.Result{}, err
				}
...
func (r *SecretReconciler) usernameFromNode(ctx context.Context, req ctrl.Request, node *core.Node) (string, error) {
