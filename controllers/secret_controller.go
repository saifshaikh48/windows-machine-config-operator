	...
	if err != nil && k8sapierrors.IsNotFound(err) {
		...
		for _, node := range nodes.Items {
			if node.Annotations[nodeconfig.PubKeyHashAnnotation] == expectedPubKeyAnno {
				// current annotation matching means that private key is still up to date
				continue
			}
			...
			if _, present := node.Annotations[BYOHAnnotation]; present {
				...
				expectedUsernameAnno, err := r.getUsernameAnno(ctx, node, privateKeyBytes)
				if err != nil {
				...
				annotationsToApply = map[string]string{
					UsernameAnnotation:              expectedUsernameAnno,
			...
			if err := r.applyAnnotations(ctx, node, annotationsToApply); err != nil {
...
// getUsernameAnno retrieves the username associated with a given node and ecrypts it using the given key
func (r *SecretReconciler) getUsernameAnno(ctx context.Context, node core.Node, key []byte) (string, error) {
	...
	encryptedUsername, err := encryptUsername(instanceUsername, key)
	if err != nil {
...
func (r *SecretReconciler) applyAnnotations(ctx context.Context, node core.Node,
	annotationsToApply map[string]string) error {
