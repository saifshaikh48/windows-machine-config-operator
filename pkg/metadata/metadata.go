...
// WaitForVersionAnnotation waits for the reboot annotation to be cleared from the node
func WaitForRebootAnnotationRemoval(ctx context.Context, c client.Client, nodeName string) error {
	...
	if err != nil {
		return fmt.Errorf("timeout waiting for %s to be cleared from node %s: %w", RebootAnnotation, nodeName, err)
	}
	return nil
}
