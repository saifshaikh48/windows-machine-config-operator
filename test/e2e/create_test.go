	...
	err = wait.Poll(nodeRetryInterval, time.Duration(math.Max(float64(nodeCount), 1))*creationTime, func() (done bool, err error) {
		...
		for _, node := range nodes {
			...
			if isBYOH && !expectError {
				...
				decUsername := strings.Replace(node.Annotations[controllers.UsernameAnnotation], "<newline>", "\n", -1)
				decUsername, err := crypto.Decrypt(decUsername, privKey)
				if err != nil {
					log.Printf("error decrypting username annotation for node %s: %s", node.Name, err)
					return false, nil
				}
				if decUsername != tc.vmUsername() {
					log.Printf("username does not match expected value for node %s:", node.Name)
					return false, nil
				}
			}
